...
if require_auth("Your Page Title"):
    ...
    # All functions should be at the top level, NOT indented under if
    ...
    def merge_csv_files(engine_df, open_tickets_df, previous_day_df):
        ...
        merged_df = pd.merge(engine_df_filtered, open_tickets_df, 
                              left_on='merchant_id_engine', right_on='stax_id_tickets', 
                              how='left', suffixes=('_engine', '_tickets'))

        merged_withsettlements = pd.merge(merged_df, previous_day_df, 
                                           left_on='merchant_id_engine', right_on='merchant_id_previous_day', 
                                           how='left')

        merged_withsettlements['matched_id'] = 'No'
        merged_withsettlements.loc[
            (merged_withsettlements['merchant_id_engine'] == merged_withsettlements['merchant_id_previous_day']), 
            'matched_id'
        ] = 'Yes'

        merged_withsettlements['matched_settlement'] = merged_withsettlements['settlement_id_engine'] \
            .isin(merged_withsettlements['settlement_id_previous_day']).map({True: 'Yes', False: 'No'})

        merged_withsettlements.drop(merged_withsettlements.columns[19:41], axis=1, inplace=True)
        merged_withsettlements.drop(columns=['merchant_id_tickets'], inplace=True)
